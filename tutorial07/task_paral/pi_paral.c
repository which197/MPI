...
long find_i_end(long num_intervals, int nproc, int rank); /*{
 ...
 * @return double 
 */
double find_integral_paral(double a, double b, long num_intervals, MPI_Comm comm)
{
    int rank, nproc;
    ...
    MPI_Allreduce(&local_estimate, &global_estimate, 1, MPI_DOUBLE, MPI_SUM, comm);
...
int task1(int argc, char **argv, MPI_Comm comm)
{
    ...
    double t1 = MPI_Wtime();
    double pi_estimate = find_integral_paral(0.0, 1.0, num_intervals, comm);
    double t2 = MPI_Wtime();
    ...
    if (rank == 0) {
        ...
        printf(GRN "Finding PI took: %.3f ms\n" RESET, (t2-t1)*1e3);
    }
